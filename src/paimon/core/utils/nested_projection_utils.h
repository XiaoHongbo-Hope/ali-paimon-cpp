...
class PAIMON_EXPORT NestedProjectionUtils {
 public:
    ...
    /// Reshape `array` (read as the file's structure) to `read_type`, null-filling
    /// nested fields added by schema evolution (a read struct that is a superset of
    /// the file struct). No-op when the types already match. STRUCT children match
    /// by paimon field id; LIST/MAP recurse into items, preserving offsets and validity.
