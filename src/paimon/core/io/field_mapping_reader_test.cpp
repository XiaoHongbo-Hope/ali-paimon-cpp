...
TEST_F(FieldMappingReaderTest, TestSchemaEvolutionAddedFieldInsideList) {
    ...
    auto read_struct = arrow::struct_({id_field("a", arrow::int32(), 10),
    ...
    ASSERT_OK_AND_ASSIGN(
        auto reader, FieldMappingReader::Create(read_schema->num_fields(), std::move(mock),
                                                BinaryRow::EmptyRow(), std::move(mapping),
                                                /*skip_map_selected_keys_filter_field_ids=*/{},
                                                pool_));
    ASSERT_OK_AND_ASSIGN(auto result_array, ReadResultCollector::CollectResult(reader.get()));

    auto expect_array = arrow::ipc::internal::json::ArrayFromJSON(
